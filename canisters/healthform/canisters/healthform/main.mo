...
actor PatientCrud {
	...
	public shared (msg) func registerPatient(nombre: Text, age: Text, pressure:Text, temperature:Text) : async () {
		let user: Text = Principal.toText(msg.caller);
		let patient = {creator=user; nombre=nombre; age=age; pressure=pressure; temperature=temperature};
	...
	public shared (msg) func updatePatient (id: Text, nombre: Text, age: Text, pressure: Text, temperature: Text,) : async Bool {
		let user: Text= Principal.toText(msg.caller);
		let patient: ?Patient = patientList.get(id);
		...
		switch (patient) {
			...
			case (?currentPatient) {
				let newPatient: Patient = {creator=user; nombre=nombre; age=age; pressure=pressure; temperature=temperature; };
	...
	public func getNumberofPatients() : async Int {
    //se obtiene el numero de elementos del HashMap como numero de pacientes
    patientList.size();
  };
